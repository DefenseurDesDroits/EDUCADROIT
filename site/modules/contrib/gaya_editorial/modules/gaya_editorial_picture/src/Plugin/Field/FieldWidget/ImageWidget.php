...
/**
 * @file
 * Contains \Drupal\gaya_editorial_picture\Plugin\Field\FieldWidget\ImageGeWidget.
 */

namespace Drupal\gaya_editorial_picture\Plugin\Field\FieldWidget;
...
/**
 * Plugin implementation of the 'image_image_ge' widget.
 *
 * @FieldWidget(
 *   id = "image_image_ge",
...
class ImageWidget extends FileWidget {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($default_image['uuid']) && $entity = \Drupal::entityManager()
  ...
  public static function process($element, FormStateInterface $form_state, $form) {
    ...
    $element['url'] = array(
      '#type' => 'textfield',
      '#title' => t('Link to an internal page or website (external page)'),
      '#default_value' => isset($item['url']) ? $item['url'] : '',
      '#description' => t('Enter the path to an internal web page: @format, or to a website: @format_domaine', array(
        '@format' => 'node/[node-id]',
        '@format_domaine' => 'http://domaine.tld'
      )),
      '#maxlength' => 1024,
      '#weight' => -11,
  ...
  /**
   * Validate callback for the URL field
   *
   * @author Sébastien Libbrecht <slibbrecht@gaya.fr>
   */
  public static function validateRequiredUrl($element, FormStateInterface $form_state) {
    if (!in_array('file_managed_file_submit', $form_state->getTriggeringElement()['#submit'])) {
      $parents = $element['#parents'];
      $image_url_field = NestedArray::getValue($form_state->getUserInput(), $parents);
      if (!UrlHelper::isValid('/' . $image_url_field)) {
        $form_state->setError($element, t('This field must be in format @format for internal page or @format_domaine for an external page', array(
          '@format' => 'node/[node-id]',
          '@format_domaine' => 'http://domaine.tld'
        )));
        return FALSE;
